...
ACCESS_TOKEN_ME = "EAACEdEose0cBAFZA4KZBbcvpWhpETxNjfFdZBsrExxQWH5vFhtFzEcIhhQqR8zHfZBU6pJ0ZBhshsO58oTRR8PARR31ob98uvp6IlQesY7mXsjmvjWULf6r2R9YyBZCJ6gYfaqZAfyaOgIZAQHcNGrFxZA4kh0j0FTtqfVvx022zSQoDZCq1EfyoW2edfZBCdf2n6YZD"
...
def generatePages(pages):
    ...
    for link in pages:
        ID = link["id"]
        url_page = "https://graph.facebook.com/"+ ID + "/feed?access_token=" + ACCESS_TOKEN_ME
        ...
        for msg in page:
            if "message" in msg and msg["id"] not in linkIDs: #if article is saved don't display
                #print msg
                retL.append({"ID": ID, "msg": msg, "name": source})
    return retL
...
def getSource(ID):
    #print ID
    url_page = "https://graph.facebook.com/"+ ID + "?fields=name&access_token=" + ACCESS_TOKEN_ME
    resp = urllib.urlopen(url_page).read()
...
def main():
    if request.method =="POST":
        ...
        source = getSource(ID)
        utils.data.save_article(ID,source, msg) #save id and msg of post
        return redirect(url_for('main'))

    if request.method == "GET":
        info = generatePages(dataSample)
